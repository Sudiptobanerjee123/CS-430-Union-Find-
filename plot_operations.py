...
def measure_performance(input_file_path):
    try:
        with open(input_file_path, 'r') as file:
            num_elements = int(file.readline().strip())  # Read the number of elements
            num_operations = int(file.readline().strip())  # Read the number of operations
            find_op, union_op = UnionFindWithPathCompression(num_elements)  # Correctly unpack the tuple here
...
def plot_performance(input_files):
    ...
    for idx, file_path in enumerate(input_files):
        ...
        if result:
            ...
            label = f'n={num_elements}, m={num_operations}, ops={operations_count}'
            labels.append(label)
            ax.bar(idx, time_taken, color=colors[idx], label=label)

    ax.set_ylabel('Time (seconds)', fontsize=12, fontweight='bold')
    ax.set_xlabel('Input Files', fontsize=12, fontweight='bold')  # Added x-axis label
    ax.set_title('Performance of Union-Find with Path Compression for Multiple Inputs', fontsize=14, fontweight='bold')  # Bold title
    ax.set_xticks(np.arange(len(input_files)))
    ax.set_xticklabels(labels, rotation=0, ha="center", fontsize=10)  # Adjust rotation angle and font size
    ax.yaxis.grid(True, linestyle='--')  # Adjust y-axis grid style
    ax.legend()
    ax.tick_params(axis='y', labelsize=10, width=2)  # Adjust y-axis label font size and make the axis bold
